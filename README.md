...
| 💡 创新思维 | [frame-breaker](skills/frame-breaker/) | 0.3.0 | 认知突破·变革式创造引擎：改变问题本身而非在框架内找更好答案——挖假设→反演→域外移植→重构→落地五步 + **思维引擎层**（8 条人类认知机制 → AI 超越杠杆）+ **保真与可验证三纪律**（不跑题/不捏造/可证伪）+ 假设审计/突破度+保真度/概念融合/类比映射/断言验证五脚本 | 已发布 |
| 👁️ 图像感知 | [image-sense](skills/image-sense/) | 0.1.0 | 无模型图像感知：纯 Python 标准库「看图」（零依赖、离线、确定性）——格式/尺寸/色深/EXIF(相机·时间·GPS)/主色/明暗/饱和度/细节/感知哈希/ASCII 预览；PNG 像素级全解析；诚实边界：不回答语义 | 已发布 |
