...
- 用脚本验证重构是否真的发生了（不是换皮），**且没脱离大意**：

```bash
python3 scripts/frame_gap.py --original "<原问题>" --reframed "<新问题>" --solution "<方案>"
python3 scripts/frame_gap.py --original "<原问题>" --reframed "<新问题>" --intent "<大意/目标/硬约束>" --solution "<方案>"
```

- 传入 `--intent`（原问题真正要解决的大意/不可违背的硬约束）会额外给出**意图保真度**：
  突破度 ≥60 但保真度 <60 = **跑题**（为变而变），回 Step 1 重审大意；
  两者都 ≥60 = **真破框且不跑题**（换的是路径，目的地还在）。
...
- **每个创造出来的新东西必须可验证**（不捏造、可溯源）——对方案里的关键断言跑验证脚手架：

```bash
python3 scripts/verify_claim.py --claim "<新方案里的关键断言>" --domain "<所属领域>"
```

- 强制回答：①断言层级（事实/合理推断/假设，证据不足就降级）②证伪清单（什么能推翻它）
  ③验证路径（做什么实验/查什么数据）④来源方向（往哪找依据，给不出就标假设）；
...
| **突破训练先验**（对齐坍缩：RLHF/DPO 把输出分布压向"最顺滑/最典型"） | 人类审美也被"最常见的答案"绑架，但人能靠直觉反叛 | 显式**稀有性选择**（DivPO 准则）：从候选池里选"稀有但高质"的，拒掉"常见但平庸"的；禁用"对齐后最典型"答案，探索训练分布外的区域 | Step 2/3 选择 |
...
> 依据：Aalto 实证「LLM 辅助重构常不提高框架质量、还容易漂离原意」+ 上下文漂移研究
> （drift 有界、可用"目标提醒"压低）+ FVA-RAG「证伪优先检索」+ OpenAI「校准弃权」+
> 溯因/反溯因引擎 + DivPO「稀有性选择」。

1. **意图保留锚（重构不脱离大意）**：进入 Step 2 前先写下一句"大意/目标/硬约束"（题面不可违背的
   前提 + 用户真正要解决的问题），重构后必须回读对照——这就是防漂移的"目标提醒"。
   机械检查：`frame_gap.py --intent` 给保真度；**突破 ≠ 跑题**，两者缺一不可。

2. **断言分级（不捏造事实）**：每个创造出来的新东西按三层标注——**事实**（可引用具体来源）/
   **合理推断**（前提显式列出，由前提推出）/**假设**（无直接证据）。证据不足的**降级为假设**
   并显式标注，禁止冒充事实。校准弃权：宁可说"这是假设，需要验证"，也不硬拗成"事实"。

3. **证伪优先（新东西必须可验证）**：每个新想法必须能答出"**什么能推翻它**"（反例/边界/竞争解释/
   逆向证据——先找"杀死查询"再找支持），以及"**怎么验证它**"（做什么实验/查什么数据/看什么文献）
   和"**往哪找依据**"（来源方向，给不出就降级假设）。跑 `verify_claim.py` 强制完成。

**为什么这是"超越人"的一部分**：人类最不可靠的两件事——自认为"没跑题"（其实漂了）和自认为"有依据"
（其实是记忆美化）。机械保真度 + 机械证伪是 AI 在这两个维度结构性超过人类的地方。

## Output Spec

- **框架对比**：`原框架 / 新框架` 对照表（必须含：假设变化、搜索空间变化、解锁了什么）；
- **反世界清单**：≥3 个，每个一句"如果……"；
- **意图保留锚**：重构前写下的大意/硬约束 + 重构后的回读对照（防跑题）；
- **断言分级**：方案里每个关键断言标注 事实/合理推断/假设，无依据的显式降级；
- **验证路径 + 来源方向**：新东西"什么能推翻它 + 怎么验证它 + 往哪找依据"；
- **落地路径**：新方案的前 30 天动作 + 最大反对理由 + 最小验证；
- **验收句**：固定格式"我把框架从『____』改成了『____』……"；
- 可选 JSON 契约：
```json
{ "old_frame": "…", "new_frame": "…", "assumptions_broken": ["…"],
  "unlocked": "…", "intent_anchor": "…", "claim_levels": {"…": "假设"},
  "falsification": ["…"], "verification_path": "…", "source_direction": "…",
  "next_30_days": "…", "frame_gap_score": 87, "intent_fidelity": 100 }
```

## Failure Modes

- **换皮式重构**：新问题只是原问题换个说法，搜索空间没变——frame_gap 突破度低就回炉；
- **跑题式重构**：突破度达标但脱离了大意/硬约束——frame_gap --intent 保真度低就回 Step 1 重审；
- **捏造式创造**：把没有依据的"假设"讲成"事实"——断言分级 + verify_claim 强制标注与证伪；
...
- `python3`（仅标准库，离线可运行）；
- `assumption_audit.py`（挖假设 + 域外规则库）/ `frame_gap.py`（突破度 + 意图保真度）
  / `concept_blend.py`（概念融合）/ `analogy_map.py`（类比映射）/ `verify_claim.py`（断言验证）。
