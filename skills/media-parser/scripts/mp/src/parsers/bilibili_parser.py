...
class BilibiliParser(BaseParser):
    ...
    def get_audio_url(self):
        """
        获取独立的音频直链：技能场景直接返回 DASH 音频流 m4s 原始地址
        （含 Referer 防盗链；如需 mp3/m4a 可自行转封装，脚本不再内置 FFmpeg 流程）。
        """
    ...
    def get_real_video_url(self):
        """
        获取 B站 durl 单文件方案（480P）的 CDN 直链。
        技能场景下直接返回 CDN 原始地址（含 Referer 防盗链，浏览器打开/带 Referer 可访问），
        不做服务器下载（上游原实现会下载到服务器再返回本地路径，技能场景不可用）。
        """
        cid = self._get_cid()
        if not self.bvid or not cid:
